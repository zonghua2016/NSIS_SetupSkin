...
通过执行 setup.exe（不要签名），先生成卸载程序，然后对卸载程序进行签名，再将卸载程序打包进 setup.exe。
- 首先，由 NSIS 生成 setup.exe，注意：不要对 setup.exe 进行签名。
- 执行 setup.exe，待安装完成后，到安装目录找到uninst.exe，然后对uninst.exe进行签名。
- 将 uninst.exe 文件与需要被释放到安装目录下的文件一起打包 (放到 FilesToInstall 目录下)。
- 注释掉 commonfunc.nsh 文件中的这句：`WriteUninstaller "$INSTDIR\uninst.exe"`。
- 重新编译 setup.nsi 生成 setup.exe，并对其进行签名。
